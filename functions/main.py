...
async def handle_feedback(request: Request):
    ...
    try:
        ...
        return JSONResponse(status_code=400, content={"error": "Format permintaan tidak valid."})
...
async def chat_endpoint(request: Request):
    ...
    try:
        ...
        if len(question) > MAX_QUESTION_LENGTH:
            return JSONResponse(status_code=400, content={"error": f"Pertanyaan terlalu panjang (maks {MAX_QUESTION_LENGTH} karakter)."})
        ...
        # Step 1: Direct Smart RAG Retrieval (compact top 4 chunks)
        kb_results = get_engine().search(question, top_k=4)
        if kb_results:
            context_text = "\n\n---\n\n".join([r.chunk[:750] for r in kb_results])
...
async def chat_stream_endpoint(request: Request):
    ...
    try:
        ...
        async def event_generator():
            ...
            # Direct Smart RAG search (compact top 4 chunks)
            kb_results = get_engine().search(question, top_k=4)
            if kb_results:
                context_text = "\n\n---\n\n".join([r.chunk[:750] for r in kb_results])
                context_block = f"\n\n==================================================\nINFORMASI RESMI SEASOLDIER:\n{context_text}\n=================================================="
            ...
            # Stream from Groq asynchronously
            full_answer = ""
            in_think = False
            ...
            try:
                ...
                try:
                    fb_res = await async_client.chat.completions.create(
        ...
        return JSONResponse(
            status_code=500,
            content={"error": "Maaf, terjadi kendala internal streaming."}
        )
